    ...
    function postProcess(buffer) {
      ...
      for (const customProperty of Object.keys(scope.customPropertyMapping)) {
        if (buffer[customProperty].length > 0) {
          geometry2.setAttribute(customProperty, new Float32BufferAttribute(buffer[customProperty], scope.customPropertyMapping[customProperty].length));
        }
      }
    ...
    function handleElement(buffer, elementName, element, cacheEntry) {
      if (elementName === "vertex") {
        ...
        if (cacheEntry.attrR !== null && cacheEntry.attrG !== null && cacheEntry.attrB !== null) {
          _color.setRGB(element[cacheEntry.attrR] / 255, element[cacheEntry.attrG] / 255, element[cacheEntry.attrB] / 255, SRGBColorSpace);
